...
const averageScale = (items: { value: string }[]) => average(items.map((item) => mapScaleToNum(item.value)).filter((value) => value > 0)) * 20;

const scoreLeadership = (value: string) => {
  const normalized = (value || "").toUpperCase();
  if (normalized.includes("EMPOWERMENT")) return 92;
  if (normalized.includes("FORMADOR") || normalized.includes("COACHING")) return 88;
  if (normalized.includes("SOPORTE")) return 82;
  if (normalized.includes("PARTICIPATIVO")) return 90;
  if (normalized.includes("CONSULTIVO")) return 78;
  if (normalized.includes("DIRECTIVO")) return 58;
  if (normalized.includes("AUTORITARIO FLEXIBLE")) return 62;
  if (normalized.includes("AUTORITARIO")) return 45;
  return mapScaleToNum(value) * 20;
};

const scoreBehavioral = (value: string) => {
  const normalized = (value || "").toUpperCase();
  if (normalized.includes("ADECUADO")) return 90;
  if (normalized.includes("OBSERVACION")) return 62;
  if (normalized.includes("DESARROLLO") || normalized.includes("DESAROLLO")) return 58;
  if (normalized.includes("RIESGO")) return 35;
  return mapScaleToNum(value) * 20;
};

const hasEvaluationData = (individual: IndividualEvaluation) => calculateCompletion(individual) > 0;

const scoreIndividual = (individual: IndividualEvaluation) => {
  const dimensions = [
    averageScale(individual.personality),
    averageScale(individual.motivational),
    averageScale(individual.teamwork),
    averageScale(individual.projective),
    scoreLeadership(individual.leadership),
    scoreBehavioral(individual.behavioral),
...
export default function FinalDashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const evaluatedIndividuals = data.individuals.filter(hasEvaluationData);
    const completionValues = evaluatedIndividuals.map(calculateCompletion);
    const individualScores = evaluatedIndividuals.map((individual) => ({
      individual,
      score: scoreIndividual(individual),
      completion: calculateCompletion(individual),
    }));
    const avgDimension = (selector: (individual: IndividualEvaluation) => number) =>
      average(evaluatedIndividuals.map(selector).filter((value) => value > 0));
    const globalRadar = [
      { subject: "Personalidad", value: avgDimension((individual) => averageScale(individual.personality)), fullMark: 100 },
      { subject: "Motivación", value: avgDimension((individual) => averageScale(individual.motivational)), fullMark: 100 },
      { subject: "Equipo", value: avgDimension((individual) => averageScale(individual.teamwork)), fullMark: 100 },
      { subject: "Proyectivo", value: avgDimension((individual) => averageScale(individual.projective)), fullMark: 100 },
      { subject: "Liderazgo", value: avgDimension((individual) => scoreLeadership(individual.leadership)), fullMark: 100 },
      { subject: "Conductual", value: avgDimension((individual) => scoreBehavioral(individual.behavioral)), fullMark: 100 },
    ];
    const priorityIndividuals = [...individualScores]
      .sort((a, b) => (a.score + a.completion * 0.35) - (b.score + b.completion * 0.35))
      .slice(0, 6);
    ...
    return {
      ...
      avgCompletion: Math.round(average(completionValues)),
      avgPotential: Math.round(average(individualScores.map((item) => item.score))),
      missingResults: data.individuals.length - evaluatedIndividuals.length,
      globalRadar,
      priorityIndividuals,
    };
  }, [data]);
      ...
      <div className={cn(
        ...
        {view === 'charts' && data && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-200 fill-mode-forwards">
            ...
            <KpiCard label="Sin Resultados" value={stats?.missingResults || 0} icon={AlertCircle} color="text-red-500" bg="bg-red-500/10" border="border-red-500/20" />
            <KpiCard label="Completitud Prom." value={`${stats?.avgCompletion || 0}%`} icon={TrendingUp} color="text-emerald-500" bg="bg-emerald-500/10" border="border-emerald-500/20" />
            <KpiCard label="Potencial Global" value={`${stats?.avgPotential || 0}%`} icon={ActivitySquare} color="text-blue-500" bg="bg-blue-500/10" border="border-blue-500/20" />
          </div>
        )}
                ...
                <CardContent className="h-[320px] sm:h-[360px] p-4 sm:p-6">
                  <ResponsiveContainer width="100%" height="100%">
                    <RadarChart data={stats?.globalRadar || []} outerRadius="75%">
                      ...
                      <Radar name="Promedio" dataKey="value" stroke="#2563eb" fill="#2563eb" fillOpacity={0.28} dot={{ r: 3, fill: "#2563eb" }} />
                ...
                <CardHeader className="border-b border-border/20 p-6">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <Badge className="bg-red-500/10 text-red-600 border-red-500/20 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest">
                      Top {stats?.priorityIndividuals?.length || 0}
                    </Badge>
                  </div>
                </CardHeader>
                <CardContent className="p-6">
                  <div className="space-y-3">
                    {(stats?.priorityIndividuals || []).map(({ individual, score, completion }, index) => (
                      <button
                        key={individual.id}
                        onClick={() => setSelectedIndividual(individual)}
                        className="w-full grid grid-cols-[38px_1fr_auto] gap-4 items-center rounded-2xl border border-border/30 bg-background/60 p-4 text-left hover:border-primary/30 hover:bg-primary/5 transition-all"
                      >
                        <div className="w-9 h-9 rounded-xl bg-muted flex items-center justify-center text-[11px] font-black text-muted-foreground">
                          {index + 1}
                        </div>
                        <div className="min-w-0">
                          <p className="text-sm font-black uppercase tracking-tight truncate">{individual.name}</p>
                          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-1">
                            {individual.leadership || "Sin liderazgo"} · {individual.behavioral || "Sin conducta"}
                          </p>
                        </div>
                        <div className="text-right min-w-[120px]">
                          <p className={cn("text-lg font-black", score < 55 ? "text-red-500" : score < 70 ? "text-amber-500" : "text-emerald-500")}>
                            {Math.round(score)}%
                          </p>
                          <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">Comp. {completion}%</p>
                        </div>
                      </button>
                    ))}
                  </div>
